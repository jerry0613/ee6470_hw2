...
void Testbench::do_GB() {
  ...
  for (y = 0; y != height + 1; ++y) {
    for (x = 0; x != width; ++x) {
      ...
      initiator.write_to_socket(GB_MM_BASE + GB_FILTER_R_ADDR, mask, data.uc, 4);
      initiator.write_to_socket(GB_MM_BASE + GB_FILTER_X_ADDR, mask, (unsigned char*)&x, 4);
      initiator.write_to_socket(GB_MM_BASE + GB_FILTER_Y_ADDR, mask, (unsigned char*)&y, 4);
      ...
      if (y >= 1 && x >= 1) {
        while(!done){
          initiator.read_from_socket(GB_MM_BASE + GB_FILTER_CHECK_ADDR, mask, data.uc, 4);
          output_num = data.uc[0] & data.uc[1] & data.uc[2];
          if(output_num>0) done=true;
        }
        //wait(1 * CLOCK_PERIOD, SC_NS);
        initiator.read_from_socket(GB_MM_BASE + GB_FILTER_RESULT_ADDR, mask,
                                  data.uc, 4);
        R = data.uc[0];
        G = data.uc[1];
        B = data.uc[2];

        *(target_bitmap + bytes_per_pixel * (width * (y - 1) + (x - 1)) + 2) = R;
        *(target_bitmap + bytes_per_pixel * (width * (y - 1) + (x - 1)) + 1) = G;
        *(target_bitmap + bytes_per_pixel * (width * (y - 1) + (x - 1)) + 0) = B;
      }
    }
  }
  sc_stop();
